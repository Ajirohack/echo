...
describe('TranslationService Integration', () => {
  ...
  test('should handle translation errors gracefully', async () => {
    ...
    await expect(translationService.translate('Hello', 'en', 'es')).rejects.toThrow(
      'Translation failed'
    );
  ...
  test('should handle language detection errors gracefully', async () => {
    ...
    await expect(translationService.detectLanguage('Test')).rejects.toThrow('Detection failed');
