...
class VirtualAudioDevice extends EventEmitter {
  ...
  async checkInstallation() {
    try {
      if (this.platform === 'win32') {
        ...
        const { stdout } = await execAsync('reg query "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\VB: Cable - A & VFX-software.com (64bit)" /v DisplayName');
      ...
      this.emit('error', new Error(`Failed to check virtual audio device installation: ${error.message}`));
      return false;
    }
  }
  ...
  async installWindows() {
    ...
    await execAsync(`powershell -Command "Start-Process -FilePath '${installerPath}' -ArgumentList '-i', '-s' -Verb RunAs -Wait"`);
    
    // Clean up
    fs.unlinkSync(downloadPath);
    fs.rmdirSync(extractPath, { recursive: true });
    
    this.emit('log', 'VB-Cable installation completed. Please restart your computer for the changes to take effect.');
  }
  ...
  async installMacOS() {
    ...
    // Download BlackHole
    const downloadUrl = 'https://github.com/ExistentialAudio/BlackHole/releases/download/2.2.9/BlackHole.2.2.9.pkg';
    ...
    this.emit('log', 'BlackHole installation completed. Please restart your computer for the changes to take effect.');
  }
  ...
  async installLinux() {
    ...
    await execAsync('sudo apt-get update && sudo apt-get install -y pulseaudio pulseaudio-utils pavucontrol');
    
    // Create a virtual sink
    await execAsync('pactl load-module module-null-sink sink_name=VirtualSink sink_properties=device.description=VirtualSink');
...
`;
  ...
  async setAsDefault() {
    ...
    try {
      ...
      this.emit('error', new Error(`Failed to set virtual audio device as default: ${error.message}`));
      return false;
    }
  }
