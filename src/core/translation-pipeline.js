...
class TranslationPipeline extends EventEmitter {
    ...
    async initialize() {
        try {
            ...
            const results = {
                stt: this.config.enableSTT ? await this.sttManager.initialize() : { success: false, disabled: true },
                translation: this.config.enableTranslation ? await this.translationManager.initialize() : { success: false, disabled: true },
                tts: this.config.enableTTS && this.ttsManager ? await this.ttsManager.initialize() : { success: false, disabled: true }
            };
    ...
    async processTranscription(sttResult) {
        try {
            ...
            this.metrics.translationLatency = (this.metrics.translationLatency * 0.8) + (translationProcessingTime * 0.2);
    ...
    buildTranslationContext(sttResult) {
        ...
        if (sttResult.confidence) {
            const confidenceLevel = sttResult.confidence >= 0.9 ? 'high' :
                (sttResult.confidence >= 0.7 ? 'medium' : 'low');
    ...
    async processTextDirect(text, fromLanguage, toLanguage, options = {}) {
        try {
            ...
            this.metrics.totalTranslations++;
            this.metrics.lastProcessingTime = totalProcessingTime;
            this.metrics.translationLatency = (this.metrics.translationLatency * 0.8) + (translationResult.processingTime * 0.2);
            this.metrics.averageLatency = (this.metrics.averageLatency * 0.8) + (totalProcessingTime * 0.2);
    ...
    updateMetrics(result) {
        ...
        if (this.metrics.averageLatency === 0) {
            ...
            this.metrics.averageLatency = (this.metrics.averageLatency * 0.8) + (this.metrics.lastProcessingTime * 0.2);
        }
