...
class GoogleTranslate extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      if (this.config.apiKey) {
        ...
        if (!keyManager.isValidApiKeyFormat('google', this.config.apiKey)) {
  ...
  async translate(text, fromLanguage, toLanguage, options = {}) {
    try {
      ...
      const detectedLanguage =
        metadata?.data?.translations?.[0]?.detectedSourceLanguage || fromLanguage;
  ...
  async translateBatch(texts, fromLanguage, toLanguage, options = {}) {
    try {
      ...
      const [translations] = await this.googleTranslateInstance.translate(texts, translateOptions);
