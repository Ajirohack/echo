...
class TranslationQuality extends EventEmitter {
    ...
    performBasicEvaluation(original, translated) {
        ...
        const hasCompleteMismatch = translatedLength < originalLength * 0.3 ||
            translatedLength > originalLength * 3;
    ...
    buildEvaluationPrompt(params) {
        ...
        let depth = '';
        if (mode === 'expert') {
            depth = 'Perform an expert-level, detailed analysis including nuanced cultural and domain-specific considerations. Examine terminology consistency, cultural appropriateness, and semantic accuracy.';
        } else if (mode === 'enhanced') {
            depth = 'Perform a thorough evaluation considering accuracy, fluency, and cultural appropriateness.';
        } else {
  ...
  "improvement_suggestions": "Brief suggestions for improving the translation"
    ...
    getBestServiceForLanguagePair(fromLanguage, toLanguage) {
        ...
        for (const [service, data] of Object.entries(benchmark.services)) {
            if (data.averageScore > bestScore && data.count >= 3) { // Minimum sample size
