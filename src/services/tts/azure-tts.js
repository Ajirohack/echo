...
class AzureTTS extends EventEmitter {
    ...
    detectGender(voiceId) {
        ...
        const femaleNames = ['jenny', 'aria', 'clara', 'zira', 'julia', 'sarah', 'natasha', 'catherine'];
    ...
    async synthesize(text, voice, options = {}) {
        ...
        try {
            ...
            if (opts.useSSML) {
                ...
                inputText = this.speechSynthesis.textToSSML(text, {
                    voice: voiceId,
                    rate: opts.rate,
                    pitch: opts.pitch,
                    volume: opts.volume,
    ...
    resolveVoiceId(voice) {
        ...
        // Try to find by name
        const voiceByName = this.availableVoices.find(v =>
        ...
        const defaultVoice = this.availableVoices.find(v => v.id === 'en-US-JennyNeural')
