...
class ElevenLabsService extends EventEmitter {
    ...
    resolveVoiceId(voice) {
        ...
        const defaultVoice = this.availableVoices.find(v => v.name === 'Rachel')
    ...
    async getVoiceByLanguage(language, gender = 'female') {
        ...
        let voice = this.availableVoices.find(v =>
            v.language && v.language.toLowerCase().includes(normalizedLang) &&
            v.gender === gender
        );
        ...
        if (!voice) {
            voice = this.availableVoices.find(v =>
                v.language === 'multilingual' && v.gender === gender
            );
        }
