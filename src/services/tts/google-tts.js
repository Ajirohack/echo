...
class GoogleTTS extends EventEmitter {
    ...
    async fetchAvailableVoices() {
        ...
        try {
            ...
            this.availableVoices = voices.map(voice => {
                ...
                // Determine gender
                const gender = voice.ssmlGender === 'FEMALE' ? 'female' :
                    voice.ssmlGender === 'MALE' ? 'male' : 'neutral';
    ...
    async synthesize(text, voice, options = {}) {
        ...
        try {
            ...
            if (opts.useSSML) {
                ...
                const ssmlText = this.speechSynthesis.textToSSML(text, {
    ...
    async resolveVoiceDetails(voice) {
        ...
        if (!voiceDetails && this.availableVoices.length > 0) {
            ...
            voiceDetails = this.availableVoices.find(v =>
                v.isWavenet && v.languageCode.startsWith('en')
