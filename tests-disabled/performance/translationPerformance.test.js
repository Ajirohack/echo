...
describe('Translation Service Performance Tests', () => {
  ...
  test('should measure concurrent translation performance', async () => {
    ...
    const testTexts = Array(CONCURRENT_REQUESTS).fill().map((_, i) => 
      TEST_TEXTS[i % TEST_TEXTS.length]
    );
    
    // Warm-up run
    await Promise.all(
      testTexts.map(text => 
        translationPipeline.translate(text, 'en', 'es')
      )
    );
  ...
  test('should handle high load', async () => {
    ...
    const testTexts = Array(HIGH_LOAD_REQUESTS).fill().map((_, i) => 
      `${TEST_TEXTS[i % TEST_TEXTS.length]} (${i})`
    );
    ...
    console.log(`- Requests per second: ${(HIGH_LOAD_REQUESTS / (totalDuration / 1000)).toFixed(2)}`);
