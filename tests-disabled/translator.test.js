...
describe('Translator Utility', () => {
  ...
  describe('translate()', () => {
    test('should translate text using the provided service', async () => {
      ...
      // Assert
      expect(mockService.translate).toHaveBeenCalledWith(
        'Hello, how are you?',
        { from: 'en', to: 'es' }
      );
    ...
    test('should throw error when service is not initialized', async () => {
      ...
      // Act & Assert
      await expect(uninitializedTranslator.translate('Hello', 'en', 'es'))
        .rejects
        .toThrow('Translation service not initialized');
    });
    ...
    test('should handle translation service errors', async () => {
      ...
      // Act & Assert
      await expect(translator.translate('Hello', 'en', 'es'))
        .rejects
        .toThrow('Translation failed: Translation service unavailable');
    });
  });
  ...
  describe('getSupportedLanguages()', () => {
    ...
    test('should throw error when service is not initialized', async () => {
      ...
      // Act & Assert
      await expect(uninitializedTranslator.getSupportedLanguages())
        .rejects
        .toThrow('Translation service not initialized');
    });
    ...
    test('should handle errors when getting supported languages', async () => {
      ...
      // Act & Assert
      await expect(translator.getSupportedLanguages())
        .rejects
        .toThrow('Failed to get supported languages: Failed to fetch languages');
    });
  });
});
